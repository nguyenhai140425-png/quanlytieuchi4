...
from master_admin.models import Event, Category, UserRole, User, EventApprovalStatus, EventProgressStatus
...
def _parse_money_value(value):
    raw_value = (value or '0').replace('.', '').replace(',', '.').strip()
    try:
        return Decimal(raw_value or '0')
...
def quan_ly_da_dien_ra_view(request):
    ...
    if request.method == 'POST':
        ...
        progress_percent_raw = request.POST.get('progress_percent') or 0
        try:
            progress_percent = int(progress_percent_raw)
        except (TypeError, ValueError):
            progress_percent = 0
        progress_percent = max(0, min(100, progress_percent))

        try:
            completion_status = int(request.POST.get('completion_status') or EventProgressStatus.NOT_STARTED)
        except (TypeError, ValueError):
            completion_status = EventProgressStatus.NOT_STARTED

        actual_amount = _parse_money_value(request.POST.get('actual_amount'))
        incomplete_reason = (request.POST.get('incomplete_reason') or '').strip()
        over_budget_note = (request.POST.get('over_budget_note') or '').strip()

        has_error = False
        if progress_percent < 100 and not incomplete_reason:
            messages.error(request, "Vui lòng nhập lý do nếu sự kiện chưa hoàn thành.")
            has_error = True
        if completion_status == EventProgressStatus.NOT_COMPLETED and not incomplete_reason:
            messages.error(request, "Vui lòng nhập lý do chưa hoàn thành.")
            has_error = True
        if actual_amount > (event.totalAmount or 0) and not over_budget_note:
            messages.error(request, "Chi phí thực tế vượt dự kiến, vui lòng nhập ghi chú vượt chi phí.")
            has_error = True

        if not has_error:
            event.progress_percent = progress_percent
            event.completion_status = completion_status
            event.actual_amount = actual_amount
            event.incomplete_reason = incomplete_reason
            event.over_budget_note = over_budget_note
            event.report_updated_at = timezone.now()
            ...
            messages.success(request, "Cập nhật báo cáo sau sự kiện thành công.")

        redirect_url = request.path
        if selected_year:
            redirect_url = f"{redirect_url}?year={selected_year}"
    ...
    context = {
        ...
        'progress_status_choices': EventProgressStatus.choices,
    }
